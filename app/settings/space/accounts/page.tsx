...
export default function AccountsPage() {
  ...
  async function handleAdd() {
    ...
    const id = uid()
    await store.persist("accounts", "add", { id, spaceId: currentId, name: addName.trim(), balance: 0, createdAt: new Date().toISOString() })
  ...
  async function handleSaveEdit(id: string) {
    ...
    await store.persist("accounts", "update", { name: editName.trim() }, id)
    store.invalidate(["accounts"])
    setEditing(null)
    ...
    <div className="flex min-h-dvh flex-col pb-24">
      ...
      <main id="main-content" className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-4 px-4 py-4">
        <Card>
          ...
          <CardContent>
            {showAdd && (
              <div className="mb-4 flex items-center gap-2 rounded-xl border border-border p-3">
                <Input value={addName} onChange={(e) => setAddName(e.target.value)} placeholder="Account name" className="h-9 text-sm flex-1" autoFocus onKeyDown={(e) => { if (e.key === "Enter") handleAdd() }} />
                <Button size="sm" onClick={handleAdd} disabled={!addName.trim()}><Check className="h-3.5 w-3.5" /></Button>
                <Button size="sm" variant="ghost" onClick={() => { setShowAdd(false); setAddName("") }}><X className="h-3.5 w-3.5" /></Button>
              </div>
            )}
                ...
                <div key={acc.id} className="flex items-center justify-between rounded-xl px-3 py-2.5 transition-colors hover:bg-secondary/50">
                  {editing === acc.id ? (
                    <div className="flex items-center gap-2 flex-1">
                      <Input value={editName} onChange={(e) => setEditName(e.target.value)} className="h-8 text-sm flex-1" autoFocus onKeyDown={(e) => { if (e.key === "Enter") handleSaveEdit(acc.id) }} />
