...
export async function GET(request: Request) {
  try {
    ...
    const rows = await query(`SELECT * FROM public.accounts WHERE space_id = $1`, [spaceId])
...
export async function POST(request: Request) {
  try {
    ...
    await query(`INSERT INTO public.accounts (${cols.join(", ")}) VALUES (${ph.join(", ")}) ON CONFLICT (id) DO UPDATE SET ${updates}`, vals)
